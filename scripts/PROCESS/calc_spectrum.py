...
#               print("Sample"+str(j)+"Round"+str(k)+": D-min ="+str(d)+"Temperature : T = "+str(temp))
   ...
   def reduce_geoms(self,infile,outdir):
      if self.notrans == True:
         self.normalize()
      else:
         self.trans2intensity()
...
#      assert(len(self.exc)==len(self.trans))
   ...
   def writeout(self, xunit, fileout, outdir):
      units = {}
      units['nm'] = 1239.8
      units['ev'] = 1.
      units['cm'] = 8065.7
   ...
   def writeoutall(self,infile,outdir,message=""):
      ...
      toprint = message
      for un in xunits:
         outfile=outdir+"absspec."+name+".n"+str(self.nsample)+'.'+un+".cross.dat"
         toprint += "\n\tPrinting spectrum in units [ "+un+", "+yunits+"] to "+outfile
         self.writeout(un, outfile,outdir)
      ...
      for un in xunits:
         outfile=outdir+"absspec."+name+".n"+str(self.nsample)+"."+un+".molar.dat"
         toprint += "\n\tPrinting spectrum in units [ "+un+", "+yunits+"] to "+outfile
         self.writeout(un, outfile,outdir)
      print(toprint)
      self.intensity = helpints

   def writegeoms(self,infile,outdir):
      name = infile.split(".")[0]
      outfile = outdir+"n"+str(self.nsample)+"."+str(self.pid)+".geoms"
...
class SpectrumBroad(Spectrum):
   ...
   def setSigma(self):
      ...
      print("Sigma set to "+str(self.sigma))
...
if spectrum.subset > 0:
   spectrum.reduce_geoms(infile,outdir)
else:
   ...
   spectrum.writeoutall(infile,outdir)
