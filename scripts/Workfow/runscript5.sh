...
for nsample in ${nsamples[@]}; do
    ...
    for indicies_dir in "${indicies_dirs[@]}"; do
        ...
        #     step=$((nstruct/(nsample+2)))
        #     echo "sampling with fixed offset $step"
        #     echo "Picking equidistant geometries from movie"
        #     ../LAUNCH/PickGeoms.sh ../../mol_$runid/geoms.xyz ../../mol_$runid/geoms_equidist.xyz $((nsample+2)) 0 $step 980160
        #     echo "$nsample Equidistant geometries successfully picked."
        #     echo -e "\n"
        ...
        #     ../PROCESS/repre_sample_2D.py -n $((nsample+1)) -N $nstates -S $nsample -c 2 -j $nproc -J 1 -w --verbose --outdir $outdir --pdfcomp KLdiv $filename --xlim=$xlim --ylim=$ylim
